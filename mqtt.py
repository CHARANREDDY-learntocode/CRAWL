...
def analyze(node_data_list_string):
    ...
    if len(deficit_nodes) > 0:
        ...
        if len(surplus_nodes) > 0:
            ...
            sorted_quotients = sorted(surplus_quotients)
            #calculate the index of least quotients index
            print(f'sorted quotients {sorted_quotients}')
            least_quotient_index = surplus_nodes_index[surplus_quotients.index(sorted_quotients[0])]
            try:
                second_least_quotient_index = surplus_nodes_index[surplus_quotients.index(sorted_quotients[1])]
            except:
                second_least_quotient_index = -1
            #make the list of least surplus quotients index
            least_surplus_quotients_index = [least_quotient_index, second_least_quotient_index]
            #form the payload string
            for node_index in range(len(node_data_list)):
                if node_index in sufficient_nodes_index:
                    payload_string += f' N{node_index+1}/V/OFF/M/OFF'
                elif node_index in surplus_nodes_index:
                    if node_index in least_surplus_quotients_index:
                        if active_motors < 3:
                            payload_string += f' N{node_index+1}/V/OFF/M/ON'
                            active_motors += 1
                        else:
                            payload_string += f' N{node_index+1}/V/OFF/M/OFF'
                    else:
                    ...
                    payload_string += f' N{node_index+1}/V/ON/M/OFF'
                else:
            ...
            print(sorted_quotients)
            #calculate the index of least quotients index
            print(f'sorted quotients {sorted_quotients}')
            least_quotient_index = sufficient_nodes_index[sufficient_quotients.index(sorted_quotients[0])]
            try:
                second_least_quotient_index = sufficient_nodes_index[sufficient_quotients.index(sorted_quotients[1])]
            except:
                second_least_quotient_index = -1
            #make the list of least surplus quotients index
            least_sufficient_quotients_index = [least_quotient_index, second_least_quotient_index]
            #form the payload string
            for node_index in range(len(node_data_list)):
                if node_index in sufficient_nodes_index:
                    if node_index in least_sufficient_quotients_index:
                        if active_motors < 3:
                            payload_string += f' N{node_index+1}/V/OFF/M/ON'
                            active_motors += 1
                        else:
